...
| Session state | `.flowr/session-*.yaml` | Dynamic work tracking (YAML) |
...
Read `.flowr/session-*.yaml` for current work state.
...
| Session file | `ls .flowr/session-*.yaml 2>/dev/null` |
