...
3. flowr/__init__.py __version__ matches pyproject.toml (if present).
"""
...
def main() -> int:
    ...
    init_py = project_root / "flowr" / "__init__.py"
    ...
    # 3. Check flowr/__init__.py __version__ matches (if present)
    ...
    if "__version__" in init_text:
        ...
        if pkg_version != version:
            print(
                f"ERROR: flowr/__init__.py __version__ ({pkg_version!r}) "
                f"does not match pyproject.toml ({version!r})"
            )
            return 1
        print("  OK: flowr/__init__.py __version__ matches")
    else:
        print("  NOTE: flowr/__init__.py has no __version__ (skipping)")
