"""Unit tests for flowr.__main__ — in-process coverage."""
...
from flowr.__main__ import build_parser, main
...
def test_build_parser_description_matches_package_metadata() -> None:
    ...
    parser = build_parser()
    expected = importlib.metadata.metadata("flowr")["Summary"]
...
def test_main_exits_0_with_help(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["flowr", "--help"])
    with pytest.raises(SystemExit) as exc_info:
        main()
...
def test_main_exits_0_with_version(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["flowr", "--version"])
    with pytest.raises(SystemExit) as exc_info:
        main()
    assert exc_info.value.code == 0
