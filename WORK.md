...
This file is a redirect. Dynamic work state is now tracked in flowr session files.

**Read `.flowr/session-*.yaml`** for the current session state.
...
<!-- This section is kept for backwards compatibility. The authoritative source is .flowr/session-*.yaml -->
