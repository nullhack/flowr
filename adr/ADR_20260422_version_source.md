...
| Should we expose a `__version__` constant in `flowr/__init__.py`? | No — creates a second source of truth that still needs `importlib.metadata` or hardcoding to populate |

## Decision

Use `importlib.metadata.version("flowr")` at runtime.
...
- **`flowr.__version__` constant in `__init__.py`**: requires a second source of truth; still needs `importlib.metadata` or hardcoding to populate it — one extra indirection with no benefit.
