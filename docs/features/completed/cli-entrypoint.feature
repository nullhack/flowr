Feature: CLI Entrypoint
  ...
  The application exposes a command-line interface via `python -m flowr`.
  ...
  implementation lives in `flowr/__main__.py` with no new dependencies — both
  ...
  Rule: Help output
    ...
    I want to run `python -m flowr --help` and see the app name, tagline, and available options
    ...
    Example: Help flag prints description and exits successfully
      ...
      Then the output contains the application name "flowr"
  ...
  Rule: Version output
    As a developer using the template
    I want to run `python -m flowr --version` and see the current version
  ...
  Rule: Unrecognised arguments
    ...
    Example: Unknown flag exits with error code
      Given the application package is installed
      When the user runs `python -m flowr --unknown-flag`
